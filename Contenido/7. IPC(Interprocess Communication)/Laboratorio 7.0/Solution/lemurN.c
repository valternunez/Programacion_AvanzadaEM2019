...
//Procedure to move baboons from North to South
void baboon_NS(char* program){
	//VARIABLES
	int semaphoreId;

	
	//Apparently we use key_t for requesting resources for our semaphores    -->   http://www.csl.mtu.edu/cs4411.ck/www/NOTES/process/shm/key.html
	...
	//Now here we use semget and the creation of an IPC to determine if there is an error or not.   --> Validation taken from PoloGarcia
	...
	printf("North Zoboomafoo with id %i is trying to cross the rope to get to the show.\n", getpid());
	...
	printf("North Zoboomafoo with id %i is trying to cross the rope but there is only %i space available. Can he do it?\n", getpid(), semctl(semaphoreId, ROPE, GETVAL, 0));
	...
	if(semctl(semaphoreId, baboonS, GETVAL, 0) == 1){
	...
	//If the rope is not full, lets continue :)
  		...
  		//Let's call the release function from header. It is the signal.
    	release(semaphoreId, baboonN, 1);
    	//We get the id from the lemur that wants to cross.
  		printf("North Zoboomafoo with id %i gets in the rope. He is trying to get to the show!\n", getpid());
...
int main(int argc, char* argv[]){
	...
	if(argc != 2){
		printf("PRODUCER NOTE: Kratt Brothers, remember that you need to ask for the exact amount of Northener Zoboomafoos you want after the program name.\n");
		return -1;
	}
	...
	//We iterate in the amount of North Lemurs given.
	for(i = 0; i < num; i++){
		...
		if(pid == 0){     //Child Process
			baboon_NS(argv[0]); //We start the northener party. Winter is coming or something like that.
		} 
	}
